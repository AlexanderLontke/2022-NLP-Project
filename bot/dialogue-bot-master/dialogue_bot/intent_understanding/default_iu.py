...
from dialogue_bot.intent_understanding.natural_language_understanding.index_er import (
    IndexExpressionRanker,
)
from dialogue_bot.intent_understanding.natural_language_understanding.nlu import (
    create_nlu_train_test_data,
    NLUTestData,
)
from dialogue_bot.intent_understanding.natural_language_understanding.phrase_er import (
    PhraseExpressionRanker,
)
from dialogue_bot.intent_understanding.natural_language_understanding.pipeline_nlu import (
    PipelineNLU,
    NLUPipelineElement,
    ExpressionUsage,
)
from dialogue_bot.intent_understanding.natural_language_understanding.rasa_nlu.rasa_nlu import (
    RasaNLU,
)
from dialogue_bot.intent_understanding.natural_language_understanding.regex_ee import (
    RegexEntityExtractor,
)
from dialogue_bot.intent_understanding.natural_language_understanding.regex_er import (
    RegexExpressionRanker,
)
...
from dialogue_bot.models.triggers.nl import (
    PhraseNLTrigger,
    AnyNLTrigger,
    FallbackNLTrigger,
)
...
class DefaultIU(IU):
    def __init__(
        ...
        self.add_nlu(
            PipelineNLU(
                ...
                [
                    NLUPipelineElement(
                        RegexExpressionRanker(env, "rasa.regex"),
                        expression_usage=ExpressionUsage.PERFECT_MATCH,
                    ),
                    NLUPipelineElement(
                        RasaNLU(env, "rasa.rasa", confidence_threshold=0.48)
                    ),
                ],
            )
        )
        ...
        self.add_nlu(
            PipelineNLU(
                ...
                [
                    NLUPipelineElement(
                        RegexExpressionRanker(env, "senttran.expression-regex"),
                        expression_usage=ExpressionUsage.PERFECT_MATCH,
                    ),
                    NLUPipelineElement(
                        IndexExpressionRanker(env, "senttran.index"),
                        expression_usage=ExpressionUsage.FILTER_TOPK,
                        expression_top_k=50,
                    ),
                    NLUPipelineElement(
                        PhraseExpressionRanker(
                            ...
                            VectorTextScorer(
                                "senttran.scorer",
                                SentenceTransformerVectorizer("senttran.senttran"),
                                False,
                            ),
                    ...
                    NLUPipelineElement(
                        RegexEntityExtractor(env, "senttran.entity-regex"),
    ...
    def _determine_nlu(
        self, user_input: "NLInput", dialogue_state: "DialogueState"
    ) -> str:
        """
    ...
    def _determine_matchable_intents(
    ...
    def _expression_ranking_to_intent_ranking(
        self, dialogue_state: "DialogueState", intent_filter, expression_ranking
    ) -> List["RankingScore"]:
        ...
        for intent_id in intent_filter:
            ...
            if (
                (intent.nl_trigger is not None)
                and (isinstance(intent.nl_trigger, PhraseNLTrigger))
                and (intent.nl_trigger.expression.id in expression_scores)
            ):
                intent_scores[intent_id] = expression_scores[
                    intent.nl_trigger.expression.id
                ]
    ...
    def _get_nl_fallback_intent(
        self, dialogue_state: "DialogueState", matchable_intents
    ) -> Optional[str]:
        ...
        for intent_id in matchable_intents:
            intent = self.env.intent(intent_id)
            if (intent.nl_trigger is not None) and isinstance(
                intent.nl_trigger, FallbackNLTrigger
            ):
    ...
    def _run_nlu(
        self, user_input: "NLInput", dialogue_state: "DialogueState", intent_filter
    ) -> "MyNLUResult":
        ...
        logger.info(
            logcolor(
                "expression",
                "NLU-Expression-Ranking (confidence-threshold={}):".format(
                    nlu_res.confidence_threshold
                ),
            )
        )
        for e in nlu_res.expression_ranking[:15]:
            logger.info(
                logcolor(
                    "expression",
                    "\t{} {}".format(
                        "+" if (e.score >= nlu_res.confidence_threshold) else "-", e
                    ),
                )
            )
        ...
        nlu_intent_ranking = self._expression_ranking_to_intent_ranking(
            dialogue_state, intent_filter, nlu_res.expression_ranking
        )

        logger.info(
            logcolor(
                "intent",
                "NLU-Intent-Ranking (confidence-threshold={}):".format(
                    nlu_res.confidence_threshold
                ),
            )
        )
    ...
    def _score_matchable_intents(
        self,
        user_input: "UserInput",
        dialogue_state: "DialogueState",
        nlu_res: Optional["MyNLUResult"],
        matchable_intents: List[str],
    ):
        ...
        for intent_id in matchable_intents:
            intent = self.env.intent(intent_id)
            score = self._score_matchable_intent(
                user_input,
                dialogue_state,
                intent,
        ...
        perfect_intents = [
            i for i, s in intent_scores.items() if isinstance(s, PerfectScore)
        ]
        if len(perfect_intents) > 0:
            intent_ranking = [
                RankingScore(i, 1.0 if isinstance(s, PerfectScore) else 0.0)
                for i, s in intent_scores.items()
            ]
            confidence_threshold = 1.0
        else:
            intent_ranking = [
                RankingScore(i, s.value) for i, s in intent_scores.items()
            ]
        ...
        logger.info(
            logcolor(
                "intent",
                "Intent-Ranking (confidence-threshold={}):".format(
        ...
        for e in intent_ranking[:15]:
            logger.info(
                logcolor(
                    "intent",
                    "\t{} {}".format(
                        "+" if (e.score >= confidence_threshold) else "-", e
                    ),
                )
            )
    ...
    def _score_matchable_intent(
        self,
        user_input: "UserInput",
        dialogue_state: "DialogueState",
        intent: "Intent",
        nlu_score: Optional[float],
    ) -> "CustomScore":
        if isinstance(user_input, NLInput):
            if (intent.nl_trigger is not None) and isinstance(
                intent.nl_trigger, AnyNLTrigger
            ):
                score = PerfectScore()
            else:
    ...
    def _extract_entities(
        ...
        if intent_id is not None:
            intent = self.env.intent(intent_id)
            entities = self._custom_extract_entities(
                intent, entities, user_input, dialogue_state
            )
    ...
    def run(
        ...
        matchable_intents = list(
            self._determine_matchable_intents(user_input, dialogue_state)
        )
        logger.info(
            logcolor(
                "intent",
                "There are {} / {} intents to consider".format(
                    len(matchable_intents), len(list(self.env.intents()))
                ),
            )
        )
        ...
        # score intents
        intent_ranking, confidence_threshold = self._score_matchable_intents(
            user_input, dialogue_state, nlu_res, matchable_intents
        )
        ...
        if len(intent_ranking) > 0:
            ...
            logger.info(
                logcolor("intent", "Top-Intent found: {}".format(top_intent_id))
            )
        else:
        ...
        if isinstance(user_input, NLInput):
            fallback_intent_id = self._get_nl_fallback_intent(
                dialogue_state, matchable_intents
            )
            if fallback_intent_id is not None:
                logger.info(
                    logcolor(
                        "intent",
                        'NL-Fallback-Intent found: "{}"'.format(fallback_intent_id),
                    )
                )
            else:
        ...
        entities = self._extract_entities(
            top_intent_id, nlu_res, user_input, dialogue_state
        )
        ...
        return IUResult(
            ...
            confidence_threshold,
            entities,
            nlu_res.nlu_result if nlu_res is not None else None,
            fallback_intent_id,
        )
    ...
    def update_entities(
        ...
        entities = self._extract_entities(
            intent_id, nlu_res, user_input, dialogue_state
        )
    ...
    def _domain_nl_expressions(self) -> Dict[str, set]:
        ...
        res = defaultdict(set)
        for intent in self.env.intents():
            if (intent.nl_trigger is None) or (
                not isinstance(intent.nl_trigger, PhraseNLTrigger)
            ):
                continue
    ...
    def _domain_test_data(self, test_data: "NLUTestData") -> Dict[str, "NLUTestData"]:
        ...
        for domain, expression_ids in domain_nl_expressions.items():
            domain_phrase_patterns = [
                p
                for p in test_data.phrase_patterns
                if p.expression_id in expression_ids
            ]
            res[domain] = NLUTestData(domain_phrase_patterns)
        return res

    def evaluate_nlus(
        self, nlu_filter: List[str] = None, intent_filter: List[str] = None
    ):
        if nlu_filter is None:
            nlu_filter = list(self.nlus.keys())

        logger.info("Evaluate NLUs: {}".format(nlu_filter))
        train_data, test_data = create_nlu_train_test_data(
            self.env, intent_filter=intent_filter, test_size=0.33
        )
        ...
        for nlu_id in nlu_filter:
            self.nlus[nlu_id].evaluate(
                train_data, domain_test_data, evaluation_id=evaluation_id
            )
