...
from dialogue_bot.nlp.text_scorer.text_scorer import (
...
class PhraseExpressionRanker(NLU):
        ...
        if retrain:
            ...
            for pattern in tqdm(training_data.phrase_patterns):
                for phrase in pattern.generate_phrases(self.env, 50):
                    ...
                    self.env.db_handler.index_object(
                        self._db_collection_name,
                        {
                            "expression_id": phrase.expression_id,
                            "text": phrase.text,
                            "annotations": serialize_annotation(
                                self.phrase_scorer.annotate(phrase.text)
                            ),
                        },
                        {"expression_id", "text"},
                    )
    ...
    def run(self, utterance: str, intent_filter: List[str] = None) -> "NLUResult":
        ...
        # prepare query
        query = self.env.db_handler.query_attr_in_value_list(
            "expression_id", expression_filter
        )
        ...
        for x in self.env.db_handler.find(self._db_collection_name, query):
            ...
            similarity = self.phrase_scorer.similarity(
                utterance_annotations, phrase_annotation
            )
            expression_scores[x["expression_id"]] = expression_scores.get(
                x["expression_id"], []
            ) + [similarity]
        ...
        return NLUResult(
            utterance, expression_ranking, self.confidence_threshold, set([])
        )
