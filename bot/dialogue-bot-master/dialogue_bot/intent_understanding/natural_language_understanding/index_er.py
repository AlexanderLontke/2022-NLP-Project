...
class IndexExpressionRanker(NLU):
    ...
    def __init__(self, env: "BotEnv", id: str, aggregation="max"):
        ...
        self._schema = Schema(expression=ID(stored=True), text=TEXT(stored=True))
        ...
        if retrain:
            ...
            for pattern in tqdm(training_data.phrase_patterns):
                ...
                for phrase in pattern.generate_phrases(self.env, 50):
                    ...
                    self.env.index_handler.index_object(
                        self._index_name,
                        {
    ...
    def run(self, utterance: str, intent_filter: List[str] = None) -> "NLUResult":
        ...
        search_query = And(
            [
                self.env.index_handler.query_attr_in_value_list(
                    "expression", expression_filter
                ),
                self.env.index_handler.more_like_this_query(
                    self._index_name, self.env.language, "text", utterance
                ),
            ]
        )
        ...
        # query
        expression_scores = {}
        for x in self.env.index_handler.find(
            self._index_name, search_query=search_query
        ):
            score, obj = x["score"], x["obj"]
            expression_scores[obj["expression"]] = expression_scores.get(
                obj["expression"], []
            ) + [score]
        ...
        return NLUResult(utterance, expression_ranking, 0.0, set([]))
