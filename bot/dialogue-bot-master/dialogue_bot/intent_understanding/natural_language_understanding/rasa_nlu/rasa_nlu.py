...
class RasaNLU(NLU):
    ...
    def _entity_to_rasa_entity(self, entity: "PhraseEntity"):
        return {
            ...
            rasa_constants.ENTITY_ATTRIBUTE_VALUE: self._value_to_rasa_value(
                entity.value
            ),
    ...
    def _phrase_to_rasa_phrase(self, phrase: "Phrase") -> Message:
        return Message(
            data={
                ...
                rasa_constants.ENTITIES: [
                    self._entity_to_rasa_entity(e) for e in phrase.entities
                ],
            }
        )

    def _create_rasa_training_examples(
        self, training_data: "NLUTrainingData"
    ) -> List["Message"]:
        phrases = set([])
    ...
    def _create_rasa_entity_synonyms(
        self, training_data: "NLUTrainingData"
    ) -> Dict[Text, Text]:
        """
        ...
        for value in values:
            for synonym in value.synonyms:
                ...
                if synonym.entity_context is not None:
                    logger.warning(
                        "Entity-contexts in synonyms are not supported: {}".format(
                            synonym
                        )
                    )
                    continue
                ...
                if synonym.case_sensitive:
                    logger.warning(
                        "Case-sensitive synonyms are not supported: {}".format(synonym)
                    )
                    continue
        ...
        for s, vs in synonym_value_mapping.items():
            if len(vs) > 1:
                logger.warning(
                    'Ambiguous synonym-value-mappings are not supported: "{}" => {}'.format(
                        s, vs
                    )
                )
            else:
    ...
    def _create_rasa_regex_features(
        self, training_data: "NLUTrainingData"
    ) -> List[Dict[Text, Text]]:
        """
        Creates a list with:
        ...
        for value in values:
            ...
            for regex_pattern in value.all_regex_patterns:
                if regex_pattern.entity_context is not None:
                    logger.warning(
                        "Entity-contexts in are not supported in RASA: {}".format(
                            regex_pattern
                        )
                    )
                    continue
        ...
        for value_id, patterns in value_patterns.items():
            for patterns2 in chunk_list(
                list(patterns), max_elems=10
            ):  # this is just to avoid very long regular expression patterns
                res.append(
                    {
                        "name": value_feature_pattern_name(
                            self._value_to_rasa_value(value_id)
                        ),
    ...
    def _create_rasa_lookup_tables(
        self, training_data: "NLUTrainingData"
    ) -> List[Dict[Text, Any]]:
        """
        Creates a list with:
    ...
    def _train(
        self,
        training_data: Optional["NLUTrainingData"],
        config_path,
        nlu_data_path,
        output_path,
        current_model_path,
        ...
        rasa_training_data.training_examples = self._create_rasa_training_examples(
            training_data
        )
        rasa_training_data.entity_synonyms = self._create_rasa_entity_synonyms(
            training_data
        )
        rasa_training_data.regex_features = self._create_rasa_regex_features(
            training_data
        )
        rasa_training_data.lookup_tables = self._create_rasa_lookup_tables(
            training_data
        )
    ...
    def _rasa_entity_value_to_value(
        self, rasa_entity: str, rasa_value: str, intent_filter: List[str]
    ) -> Optional[str]:
        ...
        x = self._entity_value_mapper.run(
            rasa_entity, rasa_value, intent_filter=intent_filter
        )
        if x is not None:
            return x["value"]
    ...
    def run(self, utterance: str, intent_filter: List[str] = None) -> "NLUResult":
        if intent_filter is None:
            intent_filter = [
                intent.id for intent in self.env.intents(intent_filter=intent_filter)
            ]
        expression_filter = [
            expression.id
        ...
        # ranking
        expression_ranking = [
            RankingScore(dct["name"], dct["confidence"])
            for dct in rasa_res["intent_ranking"]
            if dct["name"] in expression_filter
        ]
        ...
        entities = set(
            [
                ExtractedEntity(
                    ...
                    dct["end"],
                    dct["entity"],
                    self._rasa_entity_value_to_value(
                        dct["entity"], dct["value"], intent_filter
                    ),
                    utterance[dct["start"] : dct["end"]],
                    dct["confidence"]
                    if "confidence" in dct
                    else 1.0,  # TODO what else?
                    self.id,
                )
        ...
        return NLUResult(
            utterance, expression_ranking, self.confidence_threshold, entities
        )
