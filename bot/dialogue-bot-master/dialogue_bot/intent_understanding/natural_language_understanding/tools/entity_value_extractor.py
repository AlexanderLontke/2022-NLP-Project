...
from dialogue_bot.intent_understanding.iu import ExtractedEntity, remove_ambiguous_entities
...
class EntityValueExtractor(object):
        ...
        for entity in entities:
            for value in entity.values:
                ...
                for regex_pattern in value.all_regex_patterns:
                    ...
                    if (regex_pattern.entity_context is not None) and (regex_pattern.entity_context != entity.id):
                        continue

                    self._regex_patterns.add((entity.id, value.id,
    ...
    def run(self, entity_context: Optional[str], text: str, intent_filter: List[str] = None) -> List['ExtractedEntity']:
        """
        ...
        """
        # only use entities that are possible
        entity_filter = set([e.id for e in self.env.entities(intent_filter=intent_filter)])
        ...
        for (e, v, p, c) in self._regex_patterns:
            ...
            for m in re.finditer(p, text):
                candidates.append(ExtractedEntity(m.start('uifvbzdzg'), m.end('uifvbzdzg'), e, v, m.group('uifvbzdzg'), 1., None))
