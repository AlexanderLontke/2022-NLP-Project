...
class RegexExpressionRanker(NLU):
    ...
    def __init__(self, env: "BotEnv", id: str):
        ...
        self._include_expression_patterns = defaultdict(
            set
        )  # expression-id => set(regex_patterns)
        self._exclude_expression_patterns = defaultdict(
            set
        )  # expression-id => set(regex_patterns)
    ...
    def init(self, retrain: bool, training_data: Optional["NLUTrainingData"] = None):
        ...
        for expression in self.env.nl_expressions(
            intent_filter=training_data.intent_filter
        ):
            self._include_expression_patterns[expression.id].update(
                expression.regex_patterns
            )
            self._exclude_expression_patterns[expression.id].update(
                expression.exclude_regex_patterns
            )
    ...
    def run(self, utterance: str, intent_filter: List[str] = None) -> "NLUResult":
        ...
        for expression_id in expression_filter:
            match = any(
                [
                    bool(re.match(pattern, utterance))
                    for pattern in self._include_expression_patterns[expression_id]
                ]
            ) and not any(
                [
                    bool(re.match(pattern, utterance))
                    for pattern in self._exclude_expression_patterns[expression_id]
                ]
            )
            if match:
        ...
        if len(candidate_expressions) == 1:
            expression_ranking = [RankingScore(list(candidate_expressions)[0], 1.0)]

        # complete
        expression_ranking = complete_ranking(expression_ranking, expression_filter)

        return NLUResult(utterance, expression_ranking, 1.0, set([]))
