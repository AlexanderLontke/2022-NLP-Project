...
class NLUPipelineElement(object):
    ...
    def __repr__(self):
        return '({}: "{}", use_expressions={}, use_entities={}, expression_usage={})'.format(self.__class__.__name__, self.nlu.id, self.use_expressions, self.use_entities, self.expression_usage)
...
class PipelineNLU(NLU):
    ...
    def run(self, utterance: str, intent_filter: List[str] = None) -> 'NLUResult':
        ...
        for element in self.elements:
            ...
            if element.use_expressions and (perfect_expression is None):
                ...
                if element.expression_usage == ExpressionUsage.ADD_SCORES:
                    ...
                    if (len(res.expression_ranking) > 0) and (res.expression_ranking[0].score >= 1):
                        perfect_expression = res.expression_ranking[0].ref_id

                elif element.expression_usage == ExpressionUsage.FILTER_TOPK:

                    # limit intents
                    expression_ids = [s.ref_id for s in res.expression_ranking[:element.expression_top_k]]
                    curr_intent_filter = [intent.id for intent in self.env.intents(nl_expression_filter=expression_ids)]
                else:
                    raise AssertionError('Unknown ExpressionUsage "{}"'.format(element.expression_usage))
        ...
        if perfect_expression is not None:
            ...
            expression_ranking = [RankingScore(perfect_expression, 1.)]
        else:
            confidence_threshold = np.mean(confidence_thresholds)
            expression_ranking = [RankingScore(i, float(np.mean(ss))) for i, ss in expression_scores.items()]
