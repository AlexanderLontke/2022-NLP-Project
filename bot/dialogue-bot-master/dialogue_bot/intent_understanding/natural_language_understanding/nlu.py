...
def sort_expression_ranking(expression_ranking: List['RankingScore']) -> List['RankingScore']:
...
def create_nlu_train_test_data(env: 'BotEnv',
    ...
    train_phrase_patterns, test_phrase_patterns = stratify_train_test_split(phrase_patterns, lambda x: x.expression_id,
                                                                            label_test_split=test_size)

    return NLUTrainingData(intent_filter, train_phrase_patterns), NLUTestData(test_phrase_patterns)
...
class EvaluationResult(object):
    ...
    def precision(self, average='macro'):
        return precision_score(
            [t for t, p in zip(self.true_expressions, self.pred_expressions) if p != self.OTHER_CLASSNAME],
            [p for t, p in zip(self.true_expressions, self.pred_expressions) if p != self.OTHER_CLASSNAME],
            average=average
        )
...
class NLU(ABC):
    ...
    def _evaluate(self, test_data: 'NLUTestData',
                 confidence_thresholds: List[float] = np.linspace(0, 1, 100)) -> Tuple['EvaluationResult', List['EvaluationResult']]:

        logger.info('Test on test-data: {}...'.format(test_data))

        default_res = EvaluationResult(None)  # the result for the confidence-threshold determined by nlu
        ...
        for phrase_pattern in test_data.phrase_patterns:
            ...
            for test_phrase in phrase_pattern.generate_phrases(self.env, 1):
                ...
                top_expression = nlu_res.expression_ranking[0] if len(nlu_res.expression_ranking) > 0 else None

                logger.info('-'*100)
                logger.info('\tExpected expression:  {}'.format(test_phrase.expression_id))
                logger.info('\tPredicted expression: {} (score={})'.format(top_expression.ref_id, top_expression.score))
                logger.info('-'*100)
                ...
                default_res.add(
                    ...
                    conf_expression.ref_id if conf_expression is not None else EvaluationResult.OTHER_CLASSNAME,
                    end_time - start_time
                )

                # check for different confidence-thresholds
                for i, confidence_threshold in enumerate(confidence_thresholds):
                    conf_expression = nlu_res.conf_expression(confidence_threshold=confidence_threshold)
                    threshold_res[i].add(
                        test_phrase.expression_id,
                        conf_expression.ref_id if conf_expression is not None else EvaluationResult.OTHER_CLASSNAME,
                        end_time - start_time
    ...
    def evaluate(self, train_data: 'NLUTrainingData', test_data: Union['NLUTestData', Dict[str, 'NLUTestData']],
        ...
        fig = px.scatter(df, x='recall', y='precision', text='text', color='test_name',
                         hover_data=['test_name', 'threshold', 'nr_samples', 'avg_pred_time'])
        fig.update_layout(
            title='PR-Plot for NLU "<b>{}</b>":<br>'
                  '<br><b>eval-id</b>              = <b>{}</b>'
                  '<br><b>avg. pred-time (sec)</b> = <b>{:.5f}</b>'
                .format(self.id, evaluation_id, avg_pred_time)
        )
...
class NLUResult(object):
    ...
    def __init__(self, utterance: str, expression_ranking: List['RankingScore'], confidence_threshold: float,
    ...
    def conf_expression(self, confidence_threshold: float = None) -> Optional['RankingScore']:
    ...
    def plog(self):
        ...
        for e in self.expression_ranking[:15]:
            logger.info(
                logcolor('expression', '\t{} {}'.format('+' if (e.score >= self.confidence_threshold) else '-', e)))
    ...
    def to_repr_dict(self) -> dict:
        return {
            'expression_ranking': [e.to_repr_dict() for e in self.expression_ranking[:15]],
            'confidence_threshold': self.confidence_threshold,
...
class NLUTrainingData(object):
    def __init__(self, intent_filter: List[str], phrase_patterns: List['PhrasePattern']):
        self.intent_filter = set(intent_filter)
        self.phrase_patterns = set(phrase_patterns)

    def __repr__(self):
        return '({}: #intents={}, #phrase_patterns={})'.format(self.__class__.__name__, len(self.intent_filter),
...
class NLUTestData(object):
    ...
    def __repr__(self):
        return '({}: #phrase_patterns={})'.format(self.__class__.__name__, len(self.phrase_patterns))
