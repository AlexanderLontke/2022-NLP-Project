...
class BotEnv(object):
    ...
    def ensure_nl_trigger(self, intent: "Intent"):
        ...
        if (intent.nl_trigger is None) or (
            ...
            intent.nl_trigger = PhraseNLTrigger(
                NLExpression(self, intent.id + "-expression")
            )
    ...
    def register_intent_phrase_patterns(
        self, intent_id: str, phrase_pattern: Union[list, str, "PhrasePattern"]
    ):
        """
    ...
    def register_intent_regex_patterns(
        self, intent_id: str, regex_patterns: Union[list, str]
    ):
        """
    ...
    def register_intent_exclude_regex_patterns(
        self, intent_id: str, regex_patterns: Union[list, str]
    ):
        """
    ...
    def intents(
        ...
        nl_expression_filter: List[str] = None,
    ) -> List["Intent"]:
        """
        ...
        for intent in self._intents.values():
            if (intent_filter is not None) and (intent.id not in intent_filter):
                continue
            if (domain_filter is not None) and all(
                [(d not in intent.domains) for d in domain_filter]
            ):
                continue
            if (nl_expression_filter is not None) and (
                (intent.nl_trigger is None)
                or (not isinstance(intent.nl_trigger, PhraseNLTrigger))
                or (intent.nl_trigger.expression.id not in nl_expression_filter)
            ):
                continue
    ...
    def intent(self, intent_id: str) -> "Intent":
        ...
        if intent_id not in self._intents:
            raise ValueError(
                'Intent "{}" is not registered yes. Please register first!'.format(
                    logcolor("intent", intent_id)
                )
            )
        ...
        for entity in self._entities.values():
            ...
            if intent_filter is not None:
                ...
                for intent in self.intents(intent_filter=intent_filter):
                    if (intent.entity_filter is None) or (
                        entity.id in intent.entity_filter
                    ):
                        entity_used = True
                        break
                if not entity_used:
    ...
    def entity(self, entity_id: str) -> "Entity":
        ...
        if entity_id not in self._entities:
            raise ValueError(
                'Entity "{}" is not registered yes. Please register first!'.format(
                    logcolor("entity", entity_id)
                )
            )
    ...
    def values(
        self, value_filter: List[str] = None, entity_filter: List[str] = None
    ) -> List["Value"]:
        """
    ...
    def value(self, value_id: str) -> "Value":
        ...
        if value_id not in self._values:
            raise ValueError(
                'Value "{}" is not registered yes. Please register first!'.format(
                    logcolor("value", value_id)
                )
            )
    ...
    def _respond_invalid_input(
        self, bot_session: "BotSession", user_input: "UserInput", **kwargs
    ):
    ...
    def _respond_no_intent_found(
        ...
        if bot_session.iu_result.fallback_intent_id is not None:
            logger.debug(
                'No confident intent found, using fallback intent "{}"'.format(
            ...
            logger.debug(
                'No confident intent found, using "utter_intent_not_understand" response'
            )
    ...
    def _respond_valid_input(
        ...
        **kwargs
        ...
        confident_intent_found = any(
            [s.score >= confidence_threshold for s in iu_result.intent_ranking]
        )

        # no intent found
        if not confident_intent_found:
            consume_dialogue_state = self._respond_no_intent_found(
                bot_session, user_input, **kwargs
            )
        else:
            # determine best candidate group of intents within confident intents
            confident_intents = [
                s for s in iu_result.intent_ranking if s.score >= confidence_threshold
            ]

            top_confident_intents = top_similar_scores(
                confident_intents, key=lambda s: s.score, min_score=confidence_threshold
            )
            logger.info(
                logcolor(
                    "intent", "Top-Confident intents: {}".format(top_confident_intents)
                )
            )

            # check if there are intents to verify
            verify_intents = [
                s for s in top_confident_intents if self.intent(s.ref_id).verify
            ]
            verify_intents = verify_intents[: self.max_verify_nr]
            logger.info(
                logcolor("intent", "Verifiable intents:    {}".format(verify_intents))
            )
    ...
    def log(
        self,
        bot_session: "BotSession",
        initial_ds: "DialogueState",
        dispatcher: "AccumulateDispatcher",
        user_input: "UserInput",
    ):

        self.db_handler.index_object(
            self.LOGGING_COLLECTION_NAME,
            {
