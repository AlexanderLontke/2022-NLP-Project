...
def top_similar_scores(
    lst, min_score=None, key=lambda l: l, alg=AgglomerativeClustering(n_clusters=2)
):
    if len(lst) <= 1:
        return lst
...
def plot_experiments(n, min_score, max_score, insert_scores=None, nr_experiments=10):
    ...
    fig, axs = plt.subplots(
        nr_experiments,
        len(clustering_algs),
        figsize=(len(clustering_algs) * 2, nr_experiments * 2),
    )
    for i in range(nr_experiments):
        ...
        for j in range(len(clustering_algs)):
            ...
            for cluster in clusters:
                ...
                axs[i, j].scatter(
                    [i for i in range(n_scores) if labels[i] == cluster],
                    [scores[i] for i in range(n_scores) if labels[i] == cluster],
                    c=color,
                )
