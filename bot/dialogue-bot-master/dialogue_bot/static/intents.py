...
class VerifyIntentIntent(Intent):
    ...
    def __init__(self, env: "BotEnv"):
        super().__init__(
            ...
            input_contexts=["verify_intent"],
            selection_trigger=AnySelectionTrigger(),
        )
    ...
    def _do_execute(self, session: "BotSession"):
        ...
        if input.selection_key == "intent":
            true_intent_id = session.dialogue_state.slot("verify_intent#intents")[
                input.selection_idx
            ]
            false_intent_ids = [
                i
                for i in session.dialogue_state.slot("verify_intent#intents")
                if i != true_intent_id
            ]
            ...
            self.env.db_handler.index_object(
                self.env.LEARNED_INPUTS_COLLECTION_NAME,
                {
            ...
            last_dialogue_state = session.dialogue_state.slot(
                "verify_intent#dialogue_state"
            )
            updated_entities = self.env.iu.update_entities(
                true_intent_id, last_iu_result, last_dialogue_state
            )
            ...
            raise AssertionError(
                'Unknown selection_key: "{}"'.format(input.selection_key)
            )
