...
def vector_similarity(vector1: np.ndarray, vector2: np.ndarray, metric='cosine') -> float:
    if metric == 'cosine':
        # cosine distance
        if np.linalg.norm(vector1) * np.linalg.norm(vector2) == 0:
            return 0
        return np.dot(vector1, vector2) * 1. / (np.linalg.norm(vector1) * np.linalg.norm(vector2))
    else:
