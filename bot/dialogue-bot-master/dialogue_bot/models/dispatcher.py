...
class Dispatcher(ABC):
    ...
    @abstractmethod
    def choice(self, bot_session: 'BotSession', choices: List['Choice'], text: str = None, **kwargs):
        """ Called when the chatbot gives a choice """
        raise NotImplementedError('Implement in subclass')

    @abstractmethod
    def media(self, bot_session: 'BotSession', media_id: str, media_type: str = None, title: str = None,
...
class MultiDispatcher(Dispatcher):
    ...
    def media(self, bot_session: 'BotSession', media_id: str, media_type: str = None, title: str = None,
              desc: str = None, **kwargs):
        for dispatcher in self.dispatchers:
            dispatcher.media(bot_session, media_id, media_type=media_type, title=title, desc=desc, **kwargs)
...
class AccumulateDispatcher(Dispatcher):
    ...
    def choice(self, bot_session: 'BotSession', choices: List['Choice'], text: str = None, **kwargs):
        self.responses.append({
            'type': 'choice',
            'choices': [{'key': c.key, 'idx': c.idx, 'text': c.text, **c.args} for c in choices],
        })
