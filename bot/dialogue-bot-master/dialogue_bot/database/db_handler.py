...
class DBHandler(object):
    ...
    def connect(self):
        ...
        try:
            ...
            logger.info('\tMongoDB server version: {}'.format(self.client.server_info()["version"]))
            logger.info('\tMongoDB databases:      {}'.format(self.client.list_database_names()))
            logger.info('\tMongoDB collections:    {}'.format(self.db.list_collection_names()))
...
class IndexHandler(object):
    ...
    def find(self, index_name: str, get_query: dict = None, search_query=None, limit=None):
        """
        Returns all objects matching a specific query.
        If query is None, then all objects are returned.
        """
    ...
    def extract_keywords(self, index_name: str, lang: str, fieldname: str, text: str, ignore_stopwords=False) -> List[
        Tuple[str, float]]:
        index_data = self._index_data[index_name]
        keywords = []
        with index_data['idx'].searcher() as searcher:
            try:
                ...
                pass
        if len(keywords) <= 0:
            logger.error('Failed to extract keywords from "{}". Using fallback method.'.format(text))
            keywords = [(w, 1.) for w in nltk.word_tokenize(text)]
            if ignore_stopwords:
                keywords = [(w, s) for w, s in keywords if w.lower() not in set(stopwords(lang))]
    ...
    def more_like_this_query(self, index_name: str, lang: str, fieldname: str, text: str, use_variations=True):
        """ This can be used as a search_query for IndexHandler.find(...) """
        keywords = self.extract_keywords(index_name, lang, fieldname, text, ignore_stopwords=False)
...
if __name__ == '__main__':
    ...
    h = IndexHandler(os.path.join(settings.PROJECT_DIRPATH, 'data', 'generated', 'deleteme'))
